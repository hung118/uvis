...
function viewDataImport(id) {
	...
	var link = '<s:url namespace="/data" action="viewDwsFileDWSFile" />';
	link = link + "?id=" + id;
...
function deleteDataImport(id) {
	...
	if (confirm("Are you sure you want to delete this record?")) {
		var link = '<s:url namespace="/data" action="deleteDwsFileDWSFile" />';
		link = link + "?id=" + id;
		window.location.href = link;		
	} 
}
...
<body>
	...
	<h1><s:text name="data.dws" /> - File Upload</h1>

	<s:form action="importDwsFileDWSFile" namespace="/data" method="post" enctype="multipart/form-data" id="fileUploadForm">
		<s:actionerror/>
		<s:actionmessage/>
